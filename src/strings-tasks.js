...
function removeLastOccurrences(str, value) {
  ...
  return (
    str.substring(0, valueLastIndex) +
    str.substring(valueLastIndex + value.length)
  );
}
...
function isPalindrome(str) {
  ...
  while (i !== j) {
    const leftChar = str.charAt(i).toLowerCase();
    const rightChar = str.charAt(j).toLowerCase();

    if (specialChars.has(leftChar)) {
      i += 1;
    } else if (specialChars.has(rightChar)) {
      j -= 1;
    } else {
...
function findLongestWord(sentence) {
  let longestWorldIndex = 0;
  let longestWorldLength = 0;

  const splittedSentence = sentence.split(/[ ?!,.]/);

  for (let i = 0; i < splittedSentence.length; i += 1) {
    const word = splittedSentence[i];
  ...
  return splittedSentence[longestWorldIndex];
}
